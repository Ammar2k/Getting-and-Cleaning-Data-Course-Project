...
library(reshape2)
melted_data <- melt(tidy_data, id = c('subject', 'activity'))
clean_mean <- dcast(data = melted_data, subject + activity ~ variable, mean)

write.table(melted_data_mean, "tidy_data.csv")
write.csv(clean_mean, "tidy_data.txt", row.names = FALSE, quote = FALSE)
